...
INTERVAL = os.getenv("INTERVAL", "1m")  # Binance soporta 1m, 3m, 5m, etc.
...
def get_futures_klines(symbol, interval='1m', limit=50):
...
def cancel_all_open_orders(symbol):
    try:
        ...
        if orders:
            print(f"🧹 Canceladas {len(orders)} órdenes abiertas")
...
if __name__ == "__main__":
    ...
    print(f"🚀 Bot scalping 1m listo para {SYMBOL} x{LEVERAGE}")
    active_side = None

    while True:
        try:
            df = get_futures_klines(SYMBOL, interval=INTERVAL, limit=50)
